...
for snapnum2 in snapkey:
    ...
    for parallelLoopIdx in range(loopLen):
        ...
        if type(dataArray) == int:
            ...
            if ratios[0] > 0. and ratios[1] > 0.: #accounts for erros in fit
                ...
                if cm[0] > -1:
                    ...
                    if len(tempPosDM) == 0:
                        ...
                        dmmass = 1.*np.sum(DMinEll)*massDMParticle
                        gasfrac = (gasMass+1.*stellarmass)/(gasMass + stellarmass + 1.*np.sum(DMinEll)*massDMParticle)
            else:
            ...
            outputTuple = (idx,overRadius,radius_ellipsoid,rotation,cm,gasMass,stellarmass,gasindex,gasID,tempAxis,ratios,evecs,staridx,dmidx,gasfrac,dmmass)
        ...
        if rank == 0:
            for i in range(len(outputData)):
                if type(outputData[i]) == type(0):
                    continue
                idx,overRadius,radius_ellipsoid,rotation,cm,gasMass,stellarmass,gasindex,gasID,tempAxis,ratios,evecs,staridx,dmidx,gasfrac,dmmass = outputData[i]
                print(idx)
                ...
                DMindicess[np.where(halo100_indices==idx)[0][0]] = dmidx
                
                if cm[0] > -1:
                    #Calculate Star mass
                    if type(pStar) != type(0):
                        tempPosStar = dx_wrap(pStar[staridx]-cm,boxSize)
                        tempPosStar = np.array([np.dot(pp,evecs.T) for pp in tempPosStar])
                        if len(tempPosStar) == 0:
                            #SIGO has no Stars
                            starIDs[np.where(halo100_indices==idx)[0][0]] = [-1]
                            ...
                            StarinEll = tempPosStar[:,0]**2/ratios[0]**2 + tempPosStar[:,1]**2/ratios[1]**2 + tempPosStar[:,2]**2 <= tempAxis**2 
                            starIDs[np.where(halo100_indices==idx)[0][0]] = IDStar[staridx][StarinEll]     
                    else:
                        #Simulation has no Stars
                        starIDs[np.where(halo100_indices==idx)[0][0]] = [-1]

                    #Calculate DM mass
                    tempPosDM = dx_wrap(pDM[dmidx]-cm,boxSize)			
                    tempPosDM = np.array([np.dot(pp,evecs.T) for pp in tempPosDM])
                    if len(tempPosDM) == 0:
                        #Edge case: SIGO has no DM
                        DMIDs[np.where(halo100_indices==idx)[0][0]] = [-1]
                        ...
                        DMinEll = tempPosDM[:,0]**2/ratios[0]**2 + tempPosDM[:,1]**2/ratios[1]**2 + tempPosDM[:,2]**2 <= tempAxis**2 
                        DMIDs[np.where(halo100_indices==idx)[0][0]] = IDDM[dmidx][DMinEll]
                else:
    ...
    if rank == 0:
        ...
        with open(filename+'shrinker_mpiCombinedMemTest'+s_res+'_'+s_vel+'_'+str(snapnum)+'_'+str(numProcesses)+'.dat','wb') as f:
